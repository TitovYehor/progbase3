...
{
    public class ExportGraphicDialog : Dialog
    {
        private User user;
        ...
        private string directoryPath = "../data/exports/graphics";
        ...
        public ExportGraphicDialog()
        {
            ...
            Button generateBtn = new Button("Generate graphic")
            {
                X = Pos.Left(dateLbl),
                Y = Pos.Bottom(firstSeparateLbl) + Pos.Percent(10),
                AutoSize = true,
            };
            generateBtn.Clicked += OnGenerateGraphic;
        ...
        private void OnGenerateGraphic()
        {
            if (this.ValidateData() == true)
            {   
                DateTime[] dates = new DateTime[]{leftDateField.Date, rightDateField.Date};

                ExportAndImportData.ExportGraphic(directoryPath, chooseNameInput.Text.ToString(), 
                                                  dates, user.id, postRepository, commentRepository);
            }
        }
